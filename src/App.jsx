...
const App = () => {
  ...
  const changeView = (newView) => {
    setView(newView);
    if(newView !== 'anatomy' && newView !== 'splash') setSelectedSection(null);
  };
    ...
    <div className={`min-h-screen ${currentTheme.bg} ${currentTheme.textPrimary} transition-colors duration-700 flex flex-col md:flex-row overflow-hidden select-none 
      ...
      <div className={`fixed inset-0 z-[300] transition-all duration-500 flex flex-col items-center justify-center overflow-hidden ${isArkMode ? 'opacity-100 backdrop-blur-md bg-slate-900/95 pointer-events-auto' : 'opacity-0 pointer-events-none'}`}>
        ...
        <div className={`relative z-50 text-cyan-400 font-mono text-center transition-all duration-700 delay-100 flex flex-col items-center ${isArkMode ? 'translate-y-0 scale-100' : 'translate-y-10 scale-90'}`}>
           ...
           <a href="https://www.c4computerconsulting.com" target="_blank" rel="noopener noreferrer" onClick={() => setIsArkMode(false)} className="group relative px-6 md:px-10 py-4 bg-cyan-950/40 border border-cyan-400/50 hover:bg-cyan-400/20 hover:border-cyan-300 rounded-xl transition-all shadow-[0_0_20px_rgba(34,211,238,0.1)] hover:shadow-[0_0_40px_rgba(34,211,238,0.4)] flex items-center gap-4 mb-6 hover:-translate-y-1">
      ...
      {view !== 'splash' && (
        <>
          <div className="absolute inset-0 z-0 overflow-hidden pointer-events-none bg-black animate-in fade-in duration-1000">
            <img src="/laflairfrontapproach.jpg" alt="Clinic Exterior" className={`w-full h-full object-cover transition-opacity duration-1000 ${isDarkMode ? 'opacity-20' : 'opacity-10'}`} onError={(e) => { e.target.style.display = 'none'; }} />
          ...
          <div className={`fixed top-0 left-0 right-0 z-50 py-3 px-4 md:px-8 ${currentTheme.glass} animate-in slide-in-from-top duration-500`}>
            <div className="w-full mx-auto flex flex-col xl:flex-row xl:items-center justify-between gap-4 xl:gap-8">
              <div className="flex items-center justify-between w-full xl:w-auto xl:justify-start gap-4 xl:gap-8">
                ...
                <div className="flex items-center gap-2 md:gap-4">
                 ...
                 <button onClick={() => setIsDarkMode(!isDarkMode)} className={`flex items-center gap-2 px-4 py-2 rounded-full border-2 transition-all duration-300 ${isDarkMode ? 'border-cyan-500/50 bg-slate-800 text-cyan-400 hover:bg-slate-700 hover:shadow-[0_0_15px_rgba(34,211,238,0.4)]' : 'border-stone-300 bg-white text-stone-600 hover:bg-stone-50 hover:shadow-[0_0_15px_rgba(0,0,0,0.1)]'}`}>
                    {isDarkMode ? <Sun size={14} className="animate-spin-slow" /> : <Moon size={14} />}
                    <span className="text-[9px] sm:text-[10px] font-black uppercase tracking-widest hidden sm:inline">{isDarkMode ? 'DARK [LAB]' : 'LIGHT [APPLE]'}</span>
                 </button>
                </div>
              </div>
            ...
            {view === 'anatomy' && (
              <div className="w-full max-w-lg aspect-square relative animate-in zoom-in duration-1000 flex flex-col items-center">
                <div className={`absolute top-0 right-0 z-20 flex items-center gap-3 p-2 pr-4 rounded-full ${currentTheme.glass} border ${currentTheme.border} cursor-pointer hover:scale-105 transition-all`} onClick={() => { setIsXrayMode(!isXrayMode); if(chipStatus==='broken') executeRepair(); }}>
                   <div className={`w-8 h-8 rounded-full flex items-center justify-center transition-colors ${isXrayMode ? currentTheme.accentBg : 'bg-stone-500/20'} ${isXrayMode ? 'text-white' : ''}`}><Search size={14} /></div>
                ...
                <div className={`absolute top-4 left-4 md:top-8 md:left-8 z-20 max-w-[220px] p-3 rounded-2xl ${currentTheme.glass} border ${currentTheme.border} shadow-lg transition-all duration-500 pointer-events-none`}>
                   <div className="flex items-start gap-2">
                      ...
                      <p className={`text-xs font-bold leading-relaxed ${currentTheme.textSecondary}`} key={hintIndex} style={{ animation: 'fade-in 0.5s ease-in-out' }}>{interactiveHints[hintIndex]}</p>
                   </div>
                </div>
                 <svg viewBox="0 0 400 500" className={`w-full h-full transition-all duration-700 ${isSparkling ? `scale-105 drop-shadow-[0_0_40px_${isDarkMode ? '#22d3ee' : '#6366f1'}]` : (isXrayMode ? currentTheme.xrayGlow : `drop-shadow-[0_0_60px_${isDarkMode ? 'rgba(34,211,238,0.2)' : 'rgba(0,0,0,0.05)'}]`)}`} style={{ touchAction: 'none' }}>
                    {isXrayMode && <rect x="50" y="100" width="300" height="400" fill={`url(#xray-grid-${isDarkMode ? 'dark' : 'light'})`} opacity="0.3" className="pointer-events-none" />}
                    <defs>
                      <pattern id="xray-grid-dark" width="20" height="20" patternUnits="userSpaceOnUse"><path d="M 20 0 L 0 0 0 20" fill="none" stroke="#22d3ee" strokeWidth="0.5" opacity="0.5"/></pattern>
                      <pattern id="xray-grid-light" width="20" height="20" patternUnits="userSpaceOnUse"><path d="M 20 0 L 0 0 0 20" fill="none" stroke="#171717" strokeWidth="0.5" opacity="0.15"/></pattern>
                    </defs>
                    ...
                    <g>
                       <path d="M 120 180 C 120 120, 160 120, 180 160 C 190 180, 210 180, 220 160 L 250 230 L 280 240 C 290 260, 280 300, 280 300 C 280 380, 260 420, 240 420 C 220 420, 220 380, 210 320 C 200 300, 190 300, 180 320 C 170 380, 170 420, 150 420 C 130 420, 120 380, 120 300 C 110 260, 110 220, 120 180 Z" fill={isXrayMode ? "transparent" : (selectedSection?.id === 'enamel' ? (isDarkMode ? "#2a3d46" : "#ffffff") : (isDarkMode ? "#1c1c1e" : "#f5f5f4"))} stroke={isXrayMode ? currentTheme.xrayLine : (isDarkMode ? "rgba(34,211,238,0.3)" : "#e5e5e0")} strokeWidth={isXrayMode ? "3" : "2"} className={`transition-all duration-700 ${isXrayMode ? '' : 'cursor-pointer hover:brightness-110'}`} onClick={!isXrayMode ? handleEnamelClick : undefined} onPointerMove={handleEnamelScrub} />
                        {(!isXrayMode && plaqueLevel > 0 && chipStatus !== 'broken') && <path d="M 140 290 C 150 260, 200 260, 210 290 C 220 330, 200 360, 170 360 C 140 360, 130 320, 140 290 Z" fill="rgba(250, 204, 21, 0.4)" style={{ opacity: plaqueLevel / 100 }} className="pointer-events-none blur-[6px] transition-opacity duration-75" />}
                        <path d="M 160 240 C 180 220, 220 220, 240 240 C 250 270, 250 300, 240 320 C 230 370, 230 400, 220 400 C 210 400, 210 370, 200 330 C 190 370, 190 400, 180 400 C 170 400, 170 370, 160 320 C 150 300, 150 270, 160 240 Z" fill={isXrayMode ? currentTheme.xrayLine : (selectedSection?.id === 'nerve' ? TOOTH_SECTIONS.NERVE.color : (isDarkMode ? "rgba(248, 113, 113, 0.15)" : "#fecaca"))} className={`transition-all duration-700 ${isXrayMode ? 'opacity-80 animate-pulse' : 'cursor-pointer hover:opacity-100'}`} onClick={(e) => { if(!isXrayMode) { e.stopPropagation(); handleInteraction(TOOTH_SECTIONS.NERVE); } }} />
                    ...
                    <path d="M 220 160 C 240 120, 280 120, 280 180 C 285 200, 285 220, 280 240 L 250 230 L 220 160 Z" fill={isXrayMode ? "transparent" : (isDarkMode ? "#1c1c1e" : "#f5f5f4")} stroke={isXrayMode ? currentTheme.xrayLine : (isDarkMode ? "rgba(34,211,238,0.3)" : "#e5e5e0")} strokeWidth={isXrayMode ? "3" : "2"} style={{ transform: `translate(${chipPos.x}px, ${chipPos.y}px) scale(${isDragging ? 1.05 : 1})`, transformOrigin: '250px 180px' }} className={`transition-all ${isDragging ? 'duration-0' : 'duration-700'} ${isXrayMode ? 'pointer-events-none' : (chipStatus !== 'broken' ? 'cursor-pointer hover:brightness-125' : 'cursor-grab active:cursor-grabbing')}`} onClick={(!isXrayMode && chipStatus !== 'broken') ? breakTooth : undefined} onPointerDown={handlePointerDown} />
                   ...
                   {STAFF_CARDS.map((s, i) => (
                      <div key={i} className={`flex flex-col overflow-hidden rounded-3xl border ${currentTheme.border} ${currentTheme.glass} shadow-xl group hover:-translate-y-2 transition-transform duration-500`}>
                         <div className="h-64 w-full overflow-hidden bg-stone-800">
                             <img src={s.image} alt={s.name} className="w-full h-full object-cover object-top group-hover:scale-105 transition-transform duration-700" onError={(e) => { e.target.onerror = null; e.target.src = 'https://ui-avatars.com/api/?name=' + s.name.replace(/ /g, '+') + '&background=random'; }} />
                         </div>
                         <div className="p-6">
          ...
          {view === 'outcomes' && (
              <div className="w-full max-w-4xl animate-in slide-in-from-bottom duration-700 pb-12 pt-8 z-10">
                 ...
                 <div className={`relative w-full rounded-3xl overflow-hidden mb-12 shadow-2xl border ${currentTheme.border} ${currentTheme.glass}`}>
                    <img src="/avoidthishavethat.jpg" alt="Clinical Outcomes" className="w-full h-auto object-cover" onError={(e) => { e.target.style.display = 'none'; }} />
                 </div>
            ...
            {view === 'connect' && (
               <div className="w-full max-w-4xl flex flex-col animate-in zoom-in duration-700 pb-12 pt-8 z-10">
                  ...
                  <div className="mt-12 flex justify-center w-full animate-in fade-in duration-1000 delay-500">
                     <button onClick={(e) => { e.preventDefault(); setIsArkMode(true); }} className={`group relative overflow-hidden px-8 py-3 rounded-full border ${currentTheme.border} ${currentTheme.glass} opacity-40 hover:opacity-100 transition-all duration-300 hover:shadow-[0_0_30px_rgba(34,211,238,0.4)] hover:border-cyan-400 hover:-translate-y-1 cursor-pointer z-50`}>
          ...
          <div className={`w-full md:w-[450px] lg:w-[500px] ${currentTheme.card} border-l ${currentTheme.border} p-10 pt-32 overflow-y-auto z-20 shadow-2xl relative shrink-0 animate-in slide-in-from-right duration-700`}>
            {selectedSection ? (
              ...
              <div className="space-y-12 animate-in fade-in duration-500">
                 ...
                 <section>
                     ...
                     <div className={`p-8 rounded-3xl border ${currentTheme.border} ${currentTheme.glass} text-center shadow-lg`}>
                        ...
                        <button onClick={() => changeView('team')} className={`px-8 py-3 rounded-full ${currentTheme.accentBgSoft} ${currentTheme.accentText} font-black text-xs uppercase tracking-widest hover:${currentTheme.accentBg} hover:text-white transition-all shadow-md`}>
                           View Profiles
                        </button>
                     </div>
                 </section>
